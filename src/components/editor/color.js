export class CodeSelector {
  constructor (quill, options) {
    ...
    this.codeSelectList = []
    this.languages =
      this.quill.options.codetypelist ||
      [
        'asp',
        'javascript',
        'html',
      ...
      function (delta, olddelta, source) {
        for (let d in delta.ops) {
          if (delta['ops'][d].hasOwnProperty('attributes')) {
            ...
            delta['ops'][d].hasOwnProperty('delete') &&
            delta['ops'][d]['delete'] === 1
          ) {
            this.onAddCodeBlock()
          }
        }
  ...
  ArrangeX () {
    for (
      let i = 0,
        len = Math.min(this.codeBlockList.length, this.codeSelectList.length);
      i < len;
      i++
    ) {
  ...
  onRemoveCodeBlock () {
    ...
    while (i < this.codeSelectList.length) {
      if (
        !this.codeBlockList[i] ||
        this.codeBlockList[i].offsetTop !==
          parseInt(this.codeSelectList[i].style.top, 10)
      ) {
  ...
  onAddCodeBlock (flag) {
    if (this.codeBlockList.length > this.codeSelectList.length) {
      for (let i = 0, len = this.codeBlockList.length; i < len; i++) {
        if (
          !this.codeSelectList[i] ||
          this.codeBlockList[i].offsetTop !==
            parseInt(this.codeSelectList[i].style.top, 10)
        ) {
          if (flag === 'inhriet') {
            let item = this.codeBlockList[i],
              cur = new CodeSelectBlock(
                item.offsetTop,
                item.offsetLeft,
                item.offsetWidth,
                this.languages,
                this.codeSelectList[i - 1].getElementsByTagName('input')[0]
                  .value
              )
            this.codeSelectList.splice(i, 0, cur)
            this.codeSelectList[i].getElementsByTagName(
              'input'
            )[0].value = this.codeSelectList[i - 1].getElementsByTagName(
              'input'
            )[0].value
...
class CodeSelectBlock {
  ...
  init (top, left, width, languages) {
    //init style
    this.container = this.itemConstructor(
      'div',
      {
        position: 'absolute',
    ...
    this.list = this.itemConstructor(
      'ul',
      {
        width: '120px',
        'max-height': '200px',
    ...
    this.input = this.itemConstructor(
      'input',
      {
        height: '23px',
        outline: 'none',
  ...
  initEvent () {
    ...
    this.input.addEventListener('keyup', e => {
      if (e.keyCode === 13) {
        let filter_list = Array.prototype.filter.call(
          this.list.children,
          item => {
